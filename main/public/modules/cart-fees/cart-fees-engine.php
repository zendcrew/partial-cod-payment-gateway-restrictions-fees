...
if ( !class_exists( 'WOOPCD_PartialCOD_Cart_Fees_Engine' ) ) {

    class WOOPCD_PartialCOD_Cart_Fees_Engine {
        ...
        private function can_apply_rule( $rule_head, $rule_conditions, $cart_data ) {
            ...
            if ( defined( 'WOOPCD_PARTIALCOD_PREMIUM' ) ) {
                $bool_val = WOOPCD_PartialCOD_Premium_Cart_Fees_Engine::can_apply_rule( $rule_head, $cart_data );
            }
            ...
            if ( has_filter( 'woopcd_partialcod/cart-fees/can-apply-rule' ) ) {
                $bool_val = apply_filters( 'woopcd_partialcod/cart-fees/can-apply-rule', $bool_val, $rule_head, $cart_data );
            }
        ...
        private function add_rule_fee( $cart_data, $rule_fee, $rule_head ) {
            ...
            if ( defined( 'WOOPCD_PARTIALCOD_PREMIUM' ) ) {
                $cart_data = WOOPCD_PartialCOD_Premium_Cart_Fees_Engine::add_rule_fee( $cart_data, $rule_fee, $rule_head );
            } else {
                $cart_data[ 'fees' ][ $rule_head[ 'rule_id' ] ] = $rule_fee;
            }

            // allows other plugins to modify fees data once added
            if ( has_filter( 'woopcd_partialcod/cart-fees/added-rule-fee' ) ) {
                if ( isset( $cart_data[ 'fees' ][ $rule_head[ 'rule_id' ] ] ) ) {
                    $cart_data[ 'fees' ][ $rule_head[ 'rule_id' ] ] = apply_filters( 'woopcd_partialcod/cart-fees/added-rule-fee', $cart_data[ 'fees' ][ $rule_head[ 'rule_id' ] ], $rule_head );
                }
            }
        ...
        private function get_rules_settings( $method_id ) {
            ...
            return WOOPCD_PartialCOD::get_option( $method_id . '_fee_rules_settings', $default );
        }

        private function get_rule_options( $method_id ) {

            $rules = array();
            foreach ( WOOPCD_PartialCOD::get_option( $method_id . '_fee_rules', array() ) as $rule ) {
                $rules[] = $rule;
            }
            return $rules;
        }

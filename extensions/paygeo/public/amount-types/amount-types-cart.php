...
if ( !class_exists( 'PGEO_PayGeo_Amount_Type_Cart' ) ) {

    class PGEO_PayGeo_Amount_Type_Cart {

        public function __construct() {
            add_filter( 'paygeo/calculate-cart_fixed-amount', array( $this, 'process_fixed_amount' ), 1, 3 );
            add_filter( 'paygeo/calculate-cart_shipping_classes-amount', array( $this, 'process_shipping_classes_amount' ), 1, 3 );
            add_filter( 'paygeo/calculate-cart_per-amount', array( $this, 'process_percentage_amount' ), 1, 3 );
        }
        ...
        public function process_fixed_amount( $amount, $amount_args, $cart_data ) {
            ...
            return PGEO_PayGeo_Amount_Types::prepare_amount( $amount, $calc_amount );
        }
        ...
        public function process_shipping_classes_amount( $amount, $amount_args, $cart_data ) {
            ...
            return PGEO_PayGeo_Amount_Types::prepare_amount( $amount, $calc_amount, $cart_items, $amount_args );
        }
        ...
        public function process_percentage_amount( $amount, $amount_args, $cart_data ) {
            ...
            $cart_totals = PGEO_PayGeo_Cart_Total_Types::get_totals( $args, $cart_data );
        ...
        private function get_items_by_shipping_classes( $cart_data, $shipping_classes_args ) {
            ...
            $shipping_class_ids = PGEO_PayGeo_Util::get_product_term_ids_by_slugs( $shipping_classes, 'product_shipping_class' );
            ...
            foreach ( $cart_data[ 'wc' ][ 'cart_items' ] as $cart_item ) {
                ...
                if ( PGEO_PayGeo_Validation_Util::validate_value_list( $item_shipping_class_id, $shipping_class_ids, $compare ) != true ) {
                    continue;
                }
    ...
    new PGEO_PayGeo_Amount_Type_Cart();
}

...
if ( !class_exists( 'PGEO_PayGeo_Cart_Discounts' ) ) {
    PGEO_PayGeo_Extension::required_paths( dirname( __FILE__ ), array( 'cart-discounts.php' ) );

    class PGEO_PayGeo_Cart_Discounts {

        private $discount_engine;

        public function __construct() {

            $this->discount_engine = new PGEO_PayGeo_Cart_Discounts_Engine();
            add_filter( 'paygeo/process-cart-data', array( $this->discount_engine, 'process_cart_data' ), 40, 1 );
        }
        ...
        public function get_taxes( $discount_taxes, $discount_key, $cart_data ) {
            ...
            if ( $discount[ 'amount_tax' ] > 0 ) {
                ...
                $d_taxes = apply_filters( 'paygeo/discount-amount-taxes', $discount[ 'taxes' ], $discount_key, $discount );
        ...
        public function update_order_fee_item( $item, $fee_key, $cart_data ) {
            ...
            $item->update_meta_data( 'pgeo_paygeo_data', $discount_data );
        }
        ...
        public function get_amount_html( $discount_html, $discount_key, $cart_data ) {
            ...
            $discount_amount = apply_filters( 'paygeo/discount-amount', $discount[ 'amount' ], $discount_key, $discount );
            $discount_amount_tax = apply_filters( 'paygeo/discount-amount-tax', $discount[ 'amount_tax' ], $discount_key, $discount );
            ...
            if ( '' != $cart_data[ 'discounts' ][ $discount_key ][ 'desc' ] ) {
                ...
                $desc_content = apply_filters( 'paygeo/discount-desc', $cart_data[ 'discounts' ][ $discount_key ][ 'desc' ], $discount_key, $discount );

                $desc = '<span class="paygeo-cart-desc paygeo-desc-mv dashicons dashicons-editor-help" title="' . wc_sanitize_tooltip( $desc_content ) . '"></span>';
            }
        ...
        public function get_coupon_data( $coupon_code, $cart_data ) {
            ...
            if ( isset( $discount[ 'coupon_code' ] ) && $coupon_code == $discount[ 'coupon_code' ] ) {
                return array(
                    'code' => $coupon_code,
                    'amount' => apply_filters( 'paygeo/discount-coupon-amount', $discount[ 'amount' ], $discount[ 'coupon_code' ], $discount[ 'key' ], $discount ),
        ...
        public function get_coupon_label( $coupon_code, $cart_data ) {
            ...
            $settings = PGEO_PayGeo::get_option( 'cart_discount_settings', array( 'replace_coupon_labels' => 'no' ) );
            ...
            if ( isset( $discount[ 'title' ] ) ) {
                ...
                $title = apply_filters( 'paygeo/discount-coupon-title', $discount[ 'title' ], $discount[ 'coupon_code' ], $discount[ 'key' ], $discount );

                if ( isset( $discount[ 'desc' ] ) && $discount[ 'desc' ] != '' ) {

                    $desc = apply_filters( 'paygeo/discount-coupon-desc', $discount[ 'desc' ], $discount[ 'coupon_code' ], $discount[ 'key' ], $discount );

                    $title = $title . '<span class="paygeo-cart-desc dashicons dashicons-editor-help" title="' . wc_sanitize_tooltip( $desc ) . '"></span>';
                }
        ...
        private function apply_discount( $discount_key, $discount ) {
            ...
            $args = array(
                'id' => $discount_key,
                'name' => apply_filters( 'paygeo/discount-title', $discount[ 'title' ], $discount_key, $discount ),
                'amount' => 0 - apply_filters( 'paygeo/discount-amount', $discount[ 'amount' ], $discount_key, $discount ),
        ...
        private function set_applied_counpons( $coupon_code ) {
            $coupons = PGEO_PayGeo_Cart::get_session_data( 'coupons', array() );
            $coupons[] = $coupon_code;
            PGEO_PayGeo_Cart::set_session_data( 'coupons', $coupons );
        }

        private function set_applied_discounts( $discount_keys ) {
            PGEO_PayGeo_Cart::set_session_data( 'discount_keys', $discount_keys );
        }
            ...
            if ( isset( $cart_data[ 'discounts' ] ) ) {
                ...
                foreach ( $cart_data[ 'discounts' ] as $key => $discount ) {
                    if ( 'yes' == $discount[ 'apply_as_coupon' ] ) {
                        $paygeo_coupons[] = $discount[ 'coupon_code' ];

...
if ( !class_exists( 'PGEO_PayGeo_Cart_Fees' ) ) {
    PGEO_PayGeo_Extension::required_paths( dirname( __FILE__ ), array( 'cart-fees.php' ) );

    class PGEO_PayGeo_Cart_Fees {

        private $fee_engine;

        public function __construct() {

            $this->fee_engine = new PGEO_PayGeo_Cart_Fees_Engine();
            add_filter( 'paygeo/process-cart-data', array( $this->fee_engine, 'process_cart_data' ), 50, 1 );
        }

        public function apply_fees( $cart_data ) {

            $settings = PGEO_PayGeo::get_option( 'cart_fee_settings', array( 'show_on_cart' => 'no' ) );
        ...
        public function get_taxes( $fee_taxes, $fee_key, $cart_data ) {
            ...
            if ( $fee[ 'amount_tax' ] > 0 ) {
                ...
                $d_taxes = apply_filters( 'paygeo/fee-amount-taxes', $fee[ 'taxes' ], $fee_key, $fee );
        ...
        public function update_order_fee_item( $item, $fee_key, $cart_data ) {
            ...
            $item->update_meta_data( 'pgeo_paygeo_data', $fee_data );
        }
        ...
        public function get_amount_html( $fee_html, $fee_key, $cart_data ) {
            ...
            $fee_amount = apply_filters( 'paygeo/fee-amount', $fee[ 'amount' ], $fee_key, $fee );
            $fee_amount_tax = apply_filters( 'paygeo/fee-amount-tax', $fee[ 'amount_tax' ], $fee_key, $fee );
            ...
            if ( '' != $cart_data[ 'fees' ][ $fee_key ][ 'desc' ] ) {
                ...
                $desc_content = apply_filters( 'paygeo/fee-desc', $cart_data[ 'fees' ][ $fee_key ][ 'desc' ], $fee_key, $fee );
        ...
        private function apply_fee( $fee_key, $fee ) {
            ...
            $args = array(
                'id' => $fee_key,
                'name' => apply_filters( 'paygeo/fee-title', $fee[ 'title' ], $fee_key, $fee ),
                'amount' => apply_filters( 'paygeo/fee-amount', $fee[ 'amount' ], $fee_key, $fee ),
        ...
        private function set_applied_fees( $fee_keys ) {
            ...
            PGEO_PayGeo_Cart::set_session_data( 'fee_keys', $fee_keys );
        }

...
if ( !class_exists( 'PGEO_PayGeo_Admin_Conditions_Cart' )&& !defined( 'PGEO_PAYGEO_PREMIUM' ) ) {
    ...
    class PGEO_PayGeo_Admin_Conditions_Cart {
        ...
        public static function get_conditions($in_list, $args) {
            $in_list[ 'prem_6' ] = esc_html__( 'Cart Total Quantity (Premium)', 'pgeo-paygeo' );

            $weight_text = str_replace( '[0]', get_option( 'woocommerce_weight_unit' ), esc_html__( 'Cart Total Weight ([0])  (Premium)', 'pgeo-paygeo' ) );
            $in_list[ 'prem_7' ] = $weight_text;

            $in_list[ 'prem_8' ] = esc_html__( 'Number Of Cart Items (Premium)', 'pgeo-paygeo' );


            $in_list[ 'prem_9' ] = esc_html__( 'Applied Coupons (Premium)', 'pgeo-paygeo' );

            if ( PGEO_PayGeo_Extension::is_risky_method( $args[ 'method_id' ] ) && 'method-options' != $args[ 'module' ] ) {
                $in_list[ 'prem_10' ] = esc_html__( 'Partial Payment Method (Premium)', 'pgeo-paygeo' );
            }

    ...
    class PGEO_PayGeo_Admin_Discount_Rules_Page {
        ...
        public static function get_method_discount_rules_page_fields( $in_fields, $section_id ) {
                        ...
                        array(
                            ...
                            'fields' => array(
                                array(
                                    ...
                                    'type' => 'select2',
                                    'column_size' => 2,
                                    'column_title' => esc_html__( 'Discounts Limit', 'zcpg-woo-paygeo' ),
                                    'tooltip' => esc_html__( 'Controls cart discounts limit', 'zcpg-woo-paygeo' ),
                                    'default' => 'no',
                                ...
                                array(
                                    ...
                                    'tooltip' => esc_html__( 'Controls discounts apply mode', 'zcpg-woo-paygeo' ),
                                    'default' => 'all',
                                    'disabled_list_filter' => 'paygeo-admin/get-disabled-list',
            ...
            $in_fields[] = array(
                ...
                'max_sections_msg' => esc_html__( 'Please upgrade to premium version in order to add more discounts', 'zcpg-woo-paygeo' ),
                ...
                'template_adder' => array(
                    'position' => 'right',
                    'show_list' => false,
                    'button_text' => esc_html__( 'New Cart Discount', 'zcpg-woo-paygeo' ),
                ),
            );
        ...
        public static function get_method_discount_rule_template( $in_templates, $repeater_args ) {
            if ( $repeater_args[ 'screen' ] == 'option-page' && $repeater_args[ 'option_name' ] == PGEO_PayGeo_Admin_Page::get_option_name() ) {
                ...
                $method_text = str_replace( '[0]', $method[ 'title' ], esc_html__( '[0] discount', 'zcpg-woo-paygeo' ) );
        ...
        private static function get_rules_modes() {
            $rules_modes = array(
                'with_others' => esc_html__( 'Apply this and other discounts', 'zcpg-woo-paygeo' ),
            );

            if ( !defined( 'PGEO_PAYGEO_PREMIUM' ) ) {
                $rules_modes[ 'prem_1' ] = esc_html__( 'Apply only this discount (Premium)', 'zcpg-woo-paygeo' );
                $rules_modes[ 'prem_2' ] = esc_html__( 'Apply if other discounts are valid (Premium)', 'zcpg-woo-paygeo' );
                $rules_modes[ 'prem_3' ] = esc_html__( 'Apply if no other valid discounts (Premium)', 'zcpg-woo-paygeo' );
            }
            return $rules_modes;
        }
        ...
        private static function get_rules_apply_methods() {
            $rules_apply_methods = array(
                'all' => esc_html__( 'Apply all valid discounts', 'zcpg-woo-paygeo' ),
            );

            if ( !defined( 'PGEO_PAYGEO_PREMIUM' ) ) {
                $rules_apply_methods[ 'prem_1' ] = esc_html__( 'Apply first valid discount (Premium)', 'zcpg-woo-paygeo' );
                $rules_apply_methods[ 'prem_2' ] = esc_html__( 'Apply last valid discount (Premium)', 'zcpg-woo-paygeo' );
                $rules_apply_methods[ 'prem_3' ] = esc_html__( 'Apply the highest valid discount (Premium)', 'zcpg-woo-paygeo' );
                $rules_apply_methods[ 'prem_4' ] = esc_html__( 'Apply the lowest valid lower discount (Premium)', 'zcpg-woo-paygeo' );
            }
            $rules_apply_methods[ 'no' ] = esc_html__( 'Do not apply any discount', 'zcpg-woo-paygeo' );
